...
import type { CombatResult } from '@/domain/arena/types'
...
export function simulateCombat(player: Character): CombatResult {
  ...
  while (playerHp > 0 && enemyHp > 0 && round < 20) {
    const playerDmg = Math.max(1, Math.floor(player.attack - enemy.defense))
    const enemyDmg = Math.max(1, Math.floor(enemy.attack - player.defense))
  ...
  let outcome: 'Vitória!' | 'Derrota!' | 'Empate.'
