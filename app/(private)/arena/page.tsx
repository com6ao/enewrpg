...
type Battle =
  | null
  | {
      id: string; enemy_name: string;
      player_hp: number; player_hp_max: number;
      enemy_hp: number; enemy_hp_max: number;
      cursor: number; status: "active" | "finished";
      winner?: "player" | "enemy" | "draw" | null;
    };

type Attrs = { level:number; str:number; dex:number; intt:number; wis:number; cha:number; con:number; luck:number };
  ...
  function AttrBox({ title, a }: { title: string; a: Attrs | null }) {
    if (!a) return (
      <div className="card" style={{ padding: 8 }}>
        <h3>{title}</h3><div className="muted">Atributos indisponíveis</div>
      </div>
    );
    return (
      <div className="card" style={{ padding: 8 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,minmax(0,1fr))", gap:6, marginTop:6, fontSize:12 }}>
          <div>STR {a.str}</div><div>DEX {a.dex}</div><div>INT {a.intt}</div><div>WIS {a.wis}</div>
          <div>CHA {a.cha}</div><div>CON {a.con}</div><div>LUCK {a.luck}</div><div />
        </div>
      </div>
    );
  }
    ...
    <main className="container" style={{ display: "grid", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        {(["creep","jungle","ancient","boss"] as const).map(a => (
          <button key={a} className="btn" disabled={state!=="idle"} onClick={()=>setArea(a)}
            style={{ background: area===a ? "#3498db" : undefined }}>{a}</button>
        ))}
        <button className="btn" onClick={startBattle} disabled={state!=="idle"}>Lutar</button>
