"use client";
type Props = { current: number; max: number; label?: string };
export default function HPBar({ current, max, label = "HP" }: Props) {
  ...
  return (
    <div style={{ width: "100%", background: "#222", borderRadius: 8, padding: 2 }}>
      ...
      <div style={{ width: "100%", height: 10, background: "#444", borderRadius: 6, overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: "#2ecc71", transition: "width 300ms linear" }} />
      </div>
    </div>
  );
}
