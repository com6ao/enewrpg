...
export async function POST(req: Request) {
  ...
  if (!enemies?.length) {
    return new NextResponse("Nenhum inimigo nesta área", { status: 400 });
  }
  ...
  // resolve tudo e guardamos para revelar por turnos
  ...
  const playerMax =
    raw?.playerMaxHp ?? raw?.player?.hpMax ?? 100;
  const enemyMax =
    raw?.enemyMaxHp ?? raw?.enemy?.hpMax ?? 100;
